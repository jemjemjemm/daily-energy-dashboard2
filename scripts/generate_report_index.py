...
def normalize_report_title(title: str, date_text: str) -> str:
    """Normalize legacy generated report titles without changing report body/content."""
    title = (title or "").strip()
    title = title.replace("Daily 유가 동향", "Daily Issue Report")
    title = title.replace("유가 동향 Report", "Daily Issue Report")
    if title in {"Daily Issue Report", "Daily Issue Report Report"}:
        return f"Daily Issue Report — {date_text}"
    return title or f"Daily Issue Report — {date_text}"
...
def title_from_file(date_text: str, html_text: str) -> str:
    ...
    if title_match:
        ...
        if title:
            return normalize_report_title(title, date_text)
    return f"Daily Issue Report — {date_text}"
...
def is_probably_report(html_text: str) -> bool:
    ...
    has_report_word = any(token in html_text for token in ("Daily Issue Report", "유가 동향", "조간 신문 트렌드", "Summary"))
